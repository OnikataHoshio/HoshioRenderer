...
vec3 BSDF(vec3 L, vec3 V, vec3 N, int MaterialSlotID)
{
...
SampleBSDFResult SampleBSDF(float xi_1, float xi_2, float xi_3, vec3 V, vec3 Ns, int MaterialSlotID)
{
    BSDFMaterial Mat = DecodeBSDFMaterial(MaterialSlotID);
...
float BSDF_PDF_Eval(vec3 wi, vec3 V, vec3 Ns, int MaterialSlotID)
{
    if (dot(Ns, wi) <= 0.0 || dot(Ns, V) <= 0.0)
        return 0.0;
...
vec3 SampleHDR(float xi_env1, float xi_env2,  vec3 V, HitResult hit_result){
    ...
    float pdfBSDF = BSDF_PDF_Eval(wi, V, Ns, hit_result.MaterialSlot);
    weight = PowerHeuristic(pdfEnv, pdfBSDF);

    return weight * envColor
         * BSDF(wi, V, Ns, hit_result.MaterialSlot)
         * NdotL
         / max(pdfEnv, 1e-6);
}
...
vec3 PathTracing(Ray ray, int maxBounce)
{
    ...
    for (int bounce = 0; bounce < maxBounce; bounce++)
    {
        ...
        sample_result = SampleBSDF(xi_BSDF1, xi_BSDF2, xi_1, V, Ns, hit_result.MaterialSlot);
        ...
        throughput *= BSDF(sample_result.wi, V, Ns, hit_result.MaterialSlot)
