...
vec3 EvalGlassTransmission(vec3 wi, vec3 wo, vec3 Ns, float eta_wi2wo, int MaterialSlotID)
{
    ...
    return vec3(1.0) * abs((1.0 - Fg) * Dg * Gg * LdotH * VdotH / denom);
}
...
vec3 PathTracing_Smooth(Ray ray, int maxBounce)
{
    vec3 finalColor = vec3(0.0);
    vec3 throughput = vec3(1.0);
    Ray currRay = ray;
    ...
    for (int bounce = 0; bounce < maxBounce; bounce++)
    {
        ...
        float eta_wo2wi = hit_result.bIsInside ? max(Mat.IOR, 1.0001)
                                               : (1.0 / max(Mat.IOR, 1.0001));

        float cosO = abs(dot(Ns, V));
        float F = FrDielectric(cosO, eta_wo2wi);

        vec3 wi = refract(-V, Ns, eta_wo2wi);
        if (dot(wi, wi) <= 1e-12)
            break;

        throughput *= vec3(1.0 - F);

        currRay.Start = hit_result.HitPoint + sign(dot(wi, Ng)) * Ng * 1e-4;
...
void main()
{
    ...
    if (gl_FragCoord.y < 0.5 * float(uResolution.y))
        uEnableVNDF = 0;
    else
        uEnableVNDF = 1;
