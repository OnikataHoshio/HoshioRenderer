...
vec3 HalfVectorTransmission(vec3 wi, vec3 wo, float eta)
{
    vec3 h = wi  + wo * eta;
    float len2 = dot(h, h);
    if (len2 <= 1e-12)
...
vec3 BRDF(vec3 L, vec3 V, vec3 N, int MaterialSlotID)
{
    ...
    float RR = 2.0 * LdotH * LdotH * Mat.Roughness;
    float Frr = RR * (FL + FV + FL * FV * (RR - 1));
    float Fd = (1.0 - 0.5 * FL) * (1.0 - 0.5 * FV) + Frr;
...
vec3 EvalGlassReflection(vec3 wi, vec3 wo, vec3 Ns, float eta, int MaterialSlotID)
{
    ...
    float Fg = FrDielectric(dot(wo, H), eta);
    float Dg = GTR2(NdotH, alpha);
    float Gg = smithG_GGX(NdotL, roughness) * smithG_GGX(NdotV, roughness);

    return Mat.BaseColor * vec3(Fg * Dg * Gg );
}

vec3 EvalGlassTransmission(vec3 wi, vec3 wo, vec3 Ns, float eta, int MaterialSlotID)
{
    if (SameHemisphere(wi, wo, Ns))
        return vec3(0.0);
    ...
    vec3 H = HalfVectorTransmission(wi, wo, eta);
    if (dot(H, H) <= 0.0)
        return vec3(0.0);
    ...
    float denom = sqr(LdotH + eta * VdotH);

    vec3 tint = sqrt(max(Mat.BaseColor, vec3(0.0)));

    return tint * abs((1.0 - Fg) * Dg * Gg * eta * eta * LdotH * VdotH
        / max( denom, 1e-6));
}
...
vec3 BSDF(vec3 wi, vec3 wo, vec3 Ns, vec3 Ng, bool bIsInside, int MaterialSlotID)
{
    ...
    float eta = bIsInside ? max(Mat.IOR, 1.0001) : (1.0 / max(Mat.IOR, 1.0001));
    bool sameSide = dot(Ng, wi) * dot(Ng, wo) > 0.0;

    vec3 f = vec3(0.0);

    f += (1.0 - Mat.Transmission) * BRDF(wi, wo, Ns, MaterialSlotID);

    if (sameSide)
        f += Mat.Transmission * EvalGlassReflection(wi, wo, Ns, eta, MaterialSlotID);
    else
        f += Mat.Transmission * EvalGlassTransmission(wi, wo, Ns, eta, MaterialSlotID);
...
float PdfGlassReflectionIS(vec3 wi, vec3 wo, vec3 Ns, float eta, float alpha)
{
    if (!SameHemisphere(wi, wo, Ns))
        return 0.0;
...
float PdfGlassTransmissionIS(vec3 wi, vec3 wo, vec3 Ns, float eta, float alpha)
{
    if (SameHemisphere(wi, wo, Ns))
        return 0.0;
    ...
    float dwh_dwi = LdotH / max(sqr(LdotH + eta * VdotH), 1e-6);

    return (1.0 - Fg) * pdfH * dwh_dwi;
}
...
float BSDF_Glass_PDF_Eval(vec3 wi, vec3 wo, vec3 Ns, bool bIsInside, int MaterialSlotID)
{
    ...
    if (SameHemisphere(wi, wo, Ns))
        return PdfGlassReflectionIS(wi, wo, Ns, eta, alpha);
    else
        return PdfGlassTransmissionIS(wi, wo, Ns, eta, alpha);
}

float BSDF_PDF_Eval(vec3 wi, vec3 wo, vec3 Ns, bool bIsInside, int MaterialSlotID)
{
    BSDFMaterial Mat = DecodeBSDFMaterial(MaterialSlotID);

    float pGlass = clamp(Mat.Transmission, 0.0, 1.0);
...
SampleBSDFResult SampleBSDF_Glass(vec3 wo, vec3 Ns, bool bIsInside, int MaterialSlotID)
{
    ...
    vec3 wi_refract = refract(-wo, H, eta);
...
SampleBSDFResult SampleBSDF(vec3 wo, vec3 Ns, vec3 Ng, bool bIsInside, int MaterialSlotID)
{
    ...
    result.cosTheta = abs(dot(Ns, result.wi));
    result.PDF = max(BSDF_PDF_Eval(result.wi, wo, Ns, bIsInside, MaterialSlotID), 1e-8);
...
vec3 SampleHDR(float xi_env1, float xi_env2, vec3 V, HitResult hit_result)
{
    ...
    float pdfBSDF = (1.0 - Mat.Transmission) * BRDF_PDF_Eval(wi, V, Ns, hit_result.MaterialSlot);
    float weight = PowerHeuristic(pdfEnv, pdfBSDF);

    return weight * envColor
         * ((1.0 - Mat.Transmission) * BRDF(wi, V, Ns, hit_result.MaterialSlot))
         * NdotL
         / max(pdfEnv, 1e-6);
}
...
vec3 PathTracing(Ray ray, int maxBounce)
{
    ...
    for (int bounce = 0; bounce < maxBounce; bounce++)
    {
        ...
        if (!hit_result.bIsHit)
        {
            if(uEnableSkybox == 1)
            {
                if (bounce == 0)
                    ...
                    float weight = PowerHeuristic(pdf_bsdf, pdfEnv);
        ...
        sample_result.bIsBTDF = false;
        sample_result.Fg = 0.0;
        ...
        throughput *= BSDF(sample_result.wi, V, Ns, Ng, hit_result.bIsInside, hit_result.MaterialSlot)
