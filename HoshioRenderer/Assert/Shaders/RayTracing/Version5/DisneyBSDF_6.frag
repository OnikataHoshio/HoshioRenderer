...
float ComputeGlassProbability(BSDFMaterial Mat)
{
...
vec3 SampleHDR(float xi_env1, float xi_env2, vec3 V, float p_glass, HitResult hit_result)
{
    ...
    float cosTheta = AbsDot(Ns, wi);
    if (cosTheta <= 1e-6)
        return vec3(0.0);

    Ray shadowRay;
    shadowRay.Start = hit_result.HitPoint + sign(dot(wi, Ng)) * Ng * 1e-4;
    ...
    float pdfBSDF = max(BSDF_PDF_Eval(
        wi, V, Ns, p_glass, hit_result.bIsInside, hit_result.MaterialSlot
    ), 1e-8);

    float weight = PowerHeuristic(pdfEnv, pdfBSDF);

    return weight * envColor
         * BSDF(wi, V, Ns, Ng, p_glass, hit_result.bIsInside, hit_result.MaterialSlot)
         * cosTheta
         / max(pdfEnv, 1e-6);
}
...
//                finalColor += throughput * SampleSkybox(currRay.Direction);
        ...
        float p_glass = ComputeGlassProbability(Mat);
        ...
        if(uEnableSkybox == 1)
        {
            finalColor += throughput * SampleHDR(xi_env1, xi_env2, V, p_glass, hit_result);
        }
