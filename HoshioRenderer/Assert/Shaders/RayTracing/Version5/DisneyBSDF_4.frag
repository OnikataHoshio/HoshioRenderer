...
vec3 SampleHDR(float xi_env1, float xi_env2, vec3 V, float p_glass, HitResult hit_result){
    ...
    float pdfBRDF = max(BSDF_PDF_Eval(wi, V, Ns, p_glass, hit_result.bIsInside, hit_result.MaterialSlot), 1e-8);
...
//        if(uEnableSkybox == 1)
//        {
//            finalColor += throughput * SampleHDR(xi_env1, xi_env2, V, sample_result.p_glass, hit_result);
//        }
