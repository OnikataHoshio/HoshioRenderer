...
const vec3 SkyColor = vec3(0.005, 0.005, 0.005);
...
float FtOutside(float cosThetaOutside, float eta)
{
    float c = clamp(cosThetaOutside, 0.0, 1.0);
    float e = max(eta, 1.0001);
    return 1.0 - FresnelDielectric(c, 1.0, e);
}

float FtIn(float cosThetaI, float eta)
{
    return FtOutside(cosThetaI, eta);
}

float FtOut(float cosThetaO, float eta)
{
    return FtOutside(cosThetaO, eta);
}
...
void DecodeBSSRDFMaterial(in int materialSlot, out vec3 baseColor, out vec3 radius, out float eta, out float scale)
{
    ...
    baseColor = max(mat.BaseColor.rgb, vec3(0.0));
    radius    = max(mat.Radius.rgb, vec3(0.0));
    eta       = mat.Eta.r;
    scale     = mat.Scale.r;
}

vec3 ComputeBurleyD(vec3 radius, float scale)
{
    return max(0.25 * scale * radius / PI, vec3(1e-4));
}


float ComputeRMax(float d)
{
    return 16.0 * max(d, 1e-4);
}

int SampleChannel(float xi, vec3 p)
{
    if (xi < p.x) return 0;
    if (xi < p.x + p.y) return 1;
    return 2;
}

float AxisProbByIndex(int axisIndex)
{
    return (axisIndex == 2) ? 0.50 : 0.25;
}

vec3 AxisByIndex(int axisIndex, vec3 T, vec3 B, vec3 N)
{
    if (axisIndex == 0) return T;
    if (axisIndex == 1) return B;
    return N;
}
                ...
                int materialSlot)
{
    ...
    vec3 d = ComputeBurleyD(radius, scale);

    float cosThetaIn  = max(dot(normalize(Ni), normalize(wi)), 0.0);
    float cosThetaOut = max(dot(normalize(No), normalize(wo)), 0.0);
    ...
    float dChannel)
{
    ...
    float d = max(dChannel, 1e-4);
    float Rmax = ComputeRMax(d);

    if (rho >= Rmax)
        return 0.0;

    float absAxisDotNg = abs(dot(axis, Ng));
    if (absAxisDotNg < 1e-6)
        return 0.0;
    ...
    return channelProb
         * axisProb
         * BurleyDiskPdf(rho, d, Rmax)
         * absAxisDotNg
         / float(hitCount);
}

float EvalPosPdfMixture(
    vec3 po,
    vec3 pi,
    vec3 Ng,
    vec3 N, vec3 T, vec3 B,
    ...
    vec3 channelProb)
{
    ...
    {
        float dChannel = dRGB[ch];
        float pChannel = channelProb[ch];

        pdf += EvalPosPdfProposal(T, po, pi, Ng, materialSlot, 0.25, pChannel, dChannel);
        pdf += EvalPosPdfProposal(B, po, pi, Ng, materialSlot, 0.25, pChannel, dChannel);
        pdf += EvalPosPdfProposal(N, po, pi, Ng, materialSlot, 0.50, pChannel, dChannel);
    }
...
SampleBSSRDFResult SampleBSSRDF(vec3 po, vec3 No, int materialSlot)
{
    ...
    result.rho = 0.0;
    result.hitCount = 0;
    result.sampledChannel = -1;

    vec3 baseColor, radius;
    float eta, scale;
    DecodeBSSRDFMaterial(materialSlot, baseColor, radius, eta, scale);

    vec3 dRGB = ComputeBurleyD(radius, scale);

    float channel_sum = baseColor.r + baseColor.g + baseColor.b;
    float invert_channel_sum = 1.0 / channel_sum;
    vec3 channelProb = baseColor * invert_channel_sum;

    int sampledChannel = SampleChannel(rand(), channelProb);
    float dSample = dRGB[sampledChannel];
    float Rmax = ComputeRMax(dSample);
    ...
    float rho = SampleBurleyRadiusTruncated(xiR, dSample, Rmax);
    ...
    vec3 wi = SampleCosineHemisphere(rand(), rand(), Ng);
    float cosTheta = max(dot(Ng, wi), 0.0);
    if (cosTheta <= 0.0)
        return result;

    float pdfDir = cosTheta / PI;

    // 关键：分母用完整的“通道 × 轴”总混合 pdf
    float pdfPos = EvalPosPdfMixture(
        po,
        h.HitPoint,
        Ng,
        N, T, B,
    ...
    result.sampledChannel = sampledChannel;
...
vec3 PathTracing(Ray ray, int maxBounce)
{
    ...
    for (int bounce = 0; bounce < maxBounce; bounce++)
    {
        ...
        SampleBSSRDFResult s = SampleBSSRDF(hit_result.HitPoint, Ng, hit_result.MaterialSlot);
        ...
        vec3 S = EvalBSSRDF(
            s.wi, s.pi, s.Ng,
            V, hit_result.HitPoint, Ng,
            hit_result.MaterialSlot
        );

        throughput *= S * s.cosTheta / max(s.pdfPos * s.pdfDir, 1e-7);
