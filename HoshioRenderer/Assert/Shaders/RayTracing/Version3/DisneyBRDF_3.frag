...
vec3 SampleHDR(float xi_1, float xi_2, vec3 V, HitResult hit_result){
    vec3 values = FetchHDRCache(xi_1, xi_2);
    ...
    float pdfOmega = pdfUV / (2.0 * PI * PI * sinThetaPolar);
...
vec3 PathTracing(Ray ray, int maxBounce)
{
    ...
    for (int bounce = 0; bounce < maxBounce; bounce++)
    {
        ...
        if(uEnableSkybox == 1)
        {
            finalColor += throughput * SampleHDR(rand(), rand(), V, hit_result);
        }
